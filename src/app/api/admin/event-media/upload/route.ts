...
export async function POST(request: Request) {
  try {
    ...
    if (
      rolesError ||
      !hasAnyRole((roles ?? []).map((item) => item.role), [
        "organizer_admin",
        "admin",
        "superadmin",
      ])
    ...
    console.error("Upload-ul media pentru eveniment a esuat neasteptat.", error);
    return NextResponse.json(
      {
        ok: false,
        message: "Imaginea nu a putut fi incarcata. Verifica sesiunea si incearca din nou.",
      },
      {
        status: 500,
        headers: withNoStoreHeaders(),
