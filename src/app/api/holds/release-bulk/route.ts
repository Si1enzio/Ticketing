...
const releaseBulkSchema = z.object({
...
export async function POST(request: Request) {
  ...
  const parsed = releaseBulkSchema.safeParse(body);
  ...
  const { data, error } = await supabase.rpc("release_multiple_seat_holds", {
    ...
    return NextResponse.json(
      {
        ...
        message: sanitizeUserFacingErrorMessage(
          error.message,
          "Locurile nu au putut fi eliberate acum.",
        ),
      },
